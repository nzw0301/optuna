...
def qei_candidates_func(
    ...
    continuous_indices = numpy.array(
        [i for i in numpy.arange(len(train_x[0])) if i not in categorical_indices]
    )
    continuous_train_x = normalize(
        train_x[:, continuous_indices], bounds=bounds[:, continuous_indices]
    )

    x_train = torch.hstack([train_x[:, categorical_indices], continuous_train_x])
    _categorical_indices = numpy.arange(len(categorical_indices))

    model = MixedSingleTaskGP(
        x_train,
        train_y,
        cat_dims=_categorical_indices
        # , outcome_transform=Standardize(m=train_y.size(-1)) # unsupported yet
        # https://botorch.org/api/_modules/botorch/models/gp_regression_mixed.html
    )
    ...
    standard_bounds = torch.zeros_like(bounds)  # (2, n_params)
    standard_bounds[1] = 1.0  # continuous' upper bounds are 1
    standard_bounds[1, : len(categorical_indices)] = bounds[
        1, categorical_indices
    ]  # replace upper bounds of categorical.

    choices = []
    for high in bounds[1, categorical_indices].T.numpy().astype(int):
        choices.append(torch.randint(high + 1, (num_choices,)))
    choices = torch.stack(choices).T  # num_choices x num_categorical_params
    fixed_features_list = []
    for choice in choices:
        fixed_features_list.append({i: value.item() for i, value in enumerate(choice)})
    ...
    candidates[:, len(categorical_indices) :] = unnormalize(
        candidates[:, len(categorical_indices) :], bounds=bounds[:, continuous_indices]
    )

    _candidates = torch.zeros_like(candidates)
    for original_index, v in zip(categorical_indices, candidates[0, :]):
        _candidates[0, original_index] = v

    for original_index, v in zip(continuous_indices, candidates[0, len(categorical_indices) :]):
        _candidates[0, original_index] = v
...
# functions.
@experimental("2.4.0")
class MixedBoTorchSampler(BaseSampler):
    ...
    def sample_relative(
        ...
        # nzw: partially revert bound values for categorical parameters.
        _bounds = numpy.zeros((len(search_space), 2))
        bound_index = 0
        ...
        for i, distribution in enumerate(search_space.values()):
            if isinstance(distribution, CategoricalDistribution):
                _bounds[i][0] = 0
                _bounds[i][1] = len(distribution.choices) - 1
                bound_index += len(distribution.choices)
                categorical_indices.append(i)
            else:
                _bounds[i] = bounds[bound_index]
                bound_index += 1

        bounds = torch.from_numpy(_bounds)
        bounds.transpose_(0, 1)

        categorical_indices = torch.from_numpy(numpy.array(categorical_indices))
        ...
        candidates = self._candidates_func(
            params, values, con, bounds, categorical_indices, self._num_choices
        )
        ...
        return custom_untransform(trans, candidates.numpy())
...
def onehot_to_int(
    search_space: _SearchSpaceTransform, trans_params: numpy.ndarray
) -> numpy.ndarray:
    """
    Convert transformed one-hot representation to integer.
    """
    assert trans_params.shape == (search_space._bounds.shape[0],)

    params = []

    for distribution, encoded_columns in zip(
        search_space._search_space.values(), search_space.column_to_encoded_columns
    ):
        trans_param = trans_params[encoded_columns]

        if isinstance(distribution, CategoricalDistribution):
            # Select the highest rated one-hot encoding.
            trans_param = trans_param.argmax()
        else:
            trans_param = trans_param[0]

        params.append(trans_param)

    return numpy.array(params)


def custom_untransform(
    search_space: _SearchSpaceTransform, trans_params: numpy.ndarray
) -> Dict[str, float]:

    params = {}

    for (name, distribution), v in zip(search_space._search_space.items(), trans_params):

        if isinstance(distribution, CategoricalDistribution):
            param = distribution.to_external_repr(v)
        else:
            param = _untransform_numerical_param(
                v.item(), distribution, search_space._transform_log
            )
